...
inspect generated manifests before deleting old local snapshots. EPIC-04, EPIC-10, EPIC-11, and
EPIC-12 production refreshes additionally require a reviewed source plan, digest-confirmed fetch,
and one complete selected snapshot-set manifest before offline replay can promote generated bytes.
Ignored snapshots may be deleted when no exact-path promotion ticket or active review needs them.
Future exact reproduction for selected catalog releases requires either the retained selected local
