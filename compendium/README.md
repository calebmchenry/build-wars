The Compendium is an in-repo documentation system for agents.

## Goals

- Self-healing: keep this documentation up to date.
- Document systems, scripts, design decisions, and QA workflows.

## Decisions

- [Project foundation](decisions/0001-project-foundation.md)
- [Runtime Guild Wars Wiki icon assets](decisions/0002-runtime-gww-icon-assets.md)

## Source And QA

- [Source policy](source-policy.md)
- [Data QA and release](data-qa-and-release.md)
- [Data ingestion platform](data-ingestion-platform.md)
- [Professions and attributes catalog](professions-and-attributes.md)
- [Skills catalog](skills-catalog.md)
- [Runes catalog](runes-catalog.md)
- [Insignias catalog](insignias-catalog.md)
- [Weapons and mods catalog](weapons-and-mods-catalog.md)
- [Equipment shell (retired prototype)](equipment-shell.md)
- [Template compatibility](template-compatibility.md)
- [Game template files](template-files.md)
- [Game rule engine](game-rule-engine.md)
- [Primary attribute effects on skill costs and timing](primary-attribute-skill-effects.md)
- [Visual prior art](visual-prior-art.md)
- [Core build editor](core-build-editor.md)
- [Focused build composer use case](build-composer-use-case.md)
- [Composer attribute adjustments (Build v4 and preview)](attribute-adjustments.md)
- [Title ranks](title-ranks.md)
- [Equipment editor (retired prototype)](equipment-editor.md)
- [Local library and sharing](local-library-and-sharing.md)
- [Multi-build workspace](multi-build-workspace.md)
